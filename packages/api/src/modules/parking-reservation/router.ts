...
import { z } from "zod";
import { getCurrentReservationDateString, toReservationDate } from "../../helpers";
...
import { markNoShowReservations } from "./application/mark-no-show-reservations";
...
export const parkingReservationRouter = {
  ...
  getAvailableParkingSpots: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      await markNoShowReservations(repository, date);
  ...
  getParkingLotUtilization: secretaryProcedure
    .input(
      z
        .object({
